...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not self.check_using_flag():
            logger.error("フラグがFalseです。admin画面のエンドポイント一覧からフラグを立ててください。")
            return
        ...
        if result:
            self.success("全てのIDのファイルへの保存に成功。")
        else:
            self.failure("ファイルへの保存時にエラー発生。")
    ...
    def check_using_flag(self):
        ...
        for name in names:
            if name not in flag_names:
                raise ValueError(f"{name}で、using=Trueが設定されていません。（フラグがFalse）")
        return True
